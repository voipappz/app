  ...
  const renderInput = (f, i) => {
    ...
    switch (f.type) {
      case 'string':
        return (
          <>
            <Select size="small" value={f.op} onChange={(e) => patchAt(i, { op: e.target.value })} sx={{ minWidth: 110 }}>
              {STRING_OPS.map((op) => <MenuItem key={op} value={op}>{t(`filters.op.${op}`, op)}</MenuItem>)}
            </Select>
            <TextField size="small" value={f.value} onChange={(e) => patchAt(i, { value: e.target.value })} placeholder={labelOf(f.name)} sx={{ flex: 1, minWidth: 140 }} />
          </>
        );
      case 'numeric':
        return (
      ...
      case 'select':
        return (
          <Select size="small" value={f.value} displayEmpty onChange={(e) => patchAt(i, { value: e.target.value })} sx={{ flex: 1, minWidth: 160 }}>
  ...
  return (
    <Paper elevation={0} sx={{ p: 1.5, mb: 2, border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: filters.length ? 1.5 : 0, flexWrap: 'wrap' }}>
        ...
        {filters.length > 0 && (
          <Button size="small" onClick={() => onChange([])} sx={{ ml: 'auto' }}>{t('filters.clear', 'Clear all')}</Button>
        )}
      </Stack>

      {filters.map((f, i) => (
        <Stack key={`${f.name}-${i}`} direction="row" spacing={1} alignItems="center" sx={{ mb: 1, flexWrap: 'wrap' }}>
          <Tooltip title={t('filters.remove', 'Remove')}>
            <IconButton size="small" onClick={() => removeAt(i)}><CloseIcon fontSize="small" /></IconButton>
          </Tooltip>
          <Typography variant="body2" sx={{ fontWeight: 600, minWidth: 90 }}>{labelOf(f.name)}</Typography>
          {renderInput(f, i)}
        </Stack>
      ))}
    </Paper>
