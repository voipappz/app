...
// Config is env-driven with switch.voipappz.io as the verified default. These
// lock the defaults and the override precedence so a stray env change is caught.
describe('loadSipConfig', () => {
  ...
  it('defaults to the verified switch.voipappz.io:8443 WSS endpoint', () => {
    expect(loadSipConfig().wssUrl).toBe('wss://switch.voipappz.io:8443');
  });

  it('derives the SIP domain from the WSS host when not set', () => {
    expect(loadSipConfig().domain).toBe('switch.voipappz.io');
  });

  it('default ICE includes the acvideo STUN server', () => {
    expect(JSON.stringify(loadSipConfig().iceServers)).toContain('stun:acvideo.voipappz.io');
  });
