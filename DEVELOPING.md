...
**The tenant owns the value and its default.** The serializer always sends a
non-empty string, defaulting to `"true"`, so changing the default is a
serializer change — no app release. The client only decodes it, with
