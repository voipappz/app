...
Pass the `embed_src` value directly to the `data-src` attribute of the `<docuseal-form>` element, or construct the URL using the `slug` key (e.g. `https://docuseal.com/s/{{ slug }}`). Either value links the embedded form in your HTML to the specific submitter created through the DocuSeal API.
